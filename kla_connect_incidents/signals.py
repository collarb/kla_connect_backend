...
from kla_connect_utils.constants import FEEDBACK_CHANGE_VERB, REJECTED_VERB, \
    ...
    MANAGER_TRANSPORT, REPORT_REVIEW_VERB, INCIDENT_REPORT_REJECTED


@receiver(post_save, sender=KlaConnectIncident)
def handle_incident_update(sender, instance, created, **kwargs):
    if not created:
        feedbackchanged = (instance.previous_feedback != instance.feedback)
        if feedbackchanged:
            notify.send(
                instance.author,
        ...
        completed = (instance.previous_status != instance.status
...
def handle_report_update(sender, instance, created, **kwargs):
    if not created:
        ...
        forwarded_for_review = (instance.previous_status != instance.status
        ...
        rejected_report = (instance.previous_status != instance.status
